0: Super quick start:
  ...
  in coding agent:
  > @H20/1-create-prompt.md "My bad prompt: Build me a website!"
  > /clear 
  > @H20/2-planner.md  @H20/01-build-personal-photo-website/
  > /clear
  > @H20/3-executor.md @H20/01-build-personal-photo-website/PLAN-01--do-this.md
  > /clear
  > @H20/3-executor.md @H20/01-build-personal-photo-website/PLAN-02--do-that.md
  > ...


1: In Claude Code or Codex (this will generate good-prompt.md):
  ...
  ----------------------------------------------------
  Outputs:
  - auto-creates next milestone dir: ./H20/NN-<kebab>/        (e.g. ./H20/01-build-personal-photo-website/)
...
OPTIONAL EXTRAS (non-core, convenience only):

  - Extras live under:
    - ./H20/Extras/
  - They are optional helpers, not part of the mean-and-lean H20 contract.


  - 2a-env-checker:
    - advisory only; scans milestone files and prints a manual checklist of likely useful environment capabilities
    - it does NOT auto-validate MCP / plugin presence; user validates manually
    Examples:
      > ./H20/Extras/2a-env-checker
      > ./H20/Extras/2a-env-checker ./H20/01-my-feature
      > ./H20/Extras/2a-env-checker ./H20/01-my-feature/PLAN-02--do-that.md


  - 3-autoexec-claude:
    - Claude-specific unattended loop for executor stage only
    - uses current user's Claude subscription login
    - re-runs Claude in a fresh session per plan
    - defaults to streaming output
    - defaults to dangerous-skip-permissions
    - stops immediately on milestone BLOCKED.md
    - if a run does not produce PLAN-NN--DONE.md, treats that as handoff / stop
    Examples:
      > ./H20/Extras/3-autoexec-claude --milestone ./H20/01-my-feature
      > ./H20/Extras/3-autoexec-claude --milestone ./01-my-feature
  ...
  - Autoexec overlay markers passed into executor:
    - AUTOEXEC_MODE=1
      - capability/tool recommendations are treated as implicit "skip"
      - if missing capability makes the plan unsafe, executor writes BLOCKED.md and stops
    - AUTOEXEC_SKIP_HUMAN=1
      - only used when --skiphuman is set
      - human-only verification is recorded as skipped after automatable setup is done
      - without it, autoexec still stops and hands off for human verification
